...
/**
 * @brief   OS_NetworkClient_socket_t contains elements that must be filled by APP to configure it
            as client.
...
/**
 * @details %OS_NetworkClientSocket_create,  Create a socket and connect to it

 * @param OS_Network_context_t:  Passed by App which is the run time context received by APP. As of now NULL

 * @param OS_NetworkClient_socket_t: Must be filled by app. Please see the struct OS_NetworkClient_socket_t for details
 ...
 * @param OS_NetworkSocket_handle_t: Handle will be filled by after the call to OS_NetworkClientSocket_create().

 * @return  SEOS_SUCCESS or SEOS_ERROR
 */

seos_err_t
OS_NetworkClientSocket_create(OS_Network_context_t ctx,
                              OS_NetworkClient_socket_t *pClientStruct,
