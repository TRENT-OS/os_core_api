 ...
 * internal RNG.
 * 
 * The following values are supported for \p type:
 * - SeosCryptoKey_Type_AES: AES symmetric key
 * 
 * The following values are supported for \p flags
 ...
 * 
 * The following values are supported for \p bits:
 * - For AES keys: 128, 192, 256
 * 
 * @param ctx (required) pointer to the seos crypto context
 * @param pKeyHandle (required) pointer to key handle
 * @param type (required) type of key to create
 * @param flags (optional) flags 
 * @param bits (required) keysize in bits
 *
 * @return an error code
 * @retval SEOS_SUCCESS if operation succeeded
 ...
 *
 * This function allocates two keys and uses the internal RNG of the crypto API
 * to fill them with the respective key data.
 * 
 * The following values are supported for \p type:
 * - SeosCryptoKey_PairType_RSA: RSA keypair
 * - SeosCryptoKey_PairType_DH: DH keypair
 * - SeosCryptoKey_PairType_SECP256r1: ECC keypair for SECP256r1 curve
 * 
 * The following values are supported for \p prvFlags and \p pubFlags:
 * - SeosCryptoKey_Flags_NONE: Key cannot be exported
 * - SeosCryptoKey_Flags_EXPORTABLE_RAW: Key is exportable in 'raw' form
 * - SeosCryptoKey_Flags_EXPORTABLE_WRAPPED: Key has to be wrapped before export
 * 
 * The following values are supported for \p bits:
 * - For RSA keypairs: 128 - 4096
 * - For DH keypairs: 64 - 4096
 * - For SECP256r1 keypairs: 256
 *
 * @param ctx (required) pointer to the seos crypto context
 ...
 * @param type (required) type of keypair to create
 * @param prvFlags (required) flags to assign to private key (typically, the private
 * key would be unexportable or only in wrapped form)
 * @param pubFlags (required) flags to assign to public key (typically, the public
 * key would be freely exportable)
 * @param bits (required) size of key in bits (e.g., 2048 for RSA)
 *
 * @return an error code
 * @retval SEOS_SUCCESS if operation succeeded
 * @retval SEOS_ERROR_INVALID_HANDLE if the object handle is invalid
 ...
 * - SeosCryptoKey_Type_RSA_PUB: RSA public key
 * - SeosCryptoKey_Type_DH_PRV: DH private key
 * - SeosCryptoKey_Type_DH_PUB: DH public key
 ...
 * - For RSA keys 128 - 4096 (size of modulus N or size of P+Q)
 * - For DH keys: 64 - 4096 (size of modulus P)
 * - For SECP256r1 keys: 256 (fixed)
 * - For AES keys: 128, 192, 256 (fixed)
 * 
 * Buffer sizes are limited to approx. 4000 bytes (PAGE_SIZE - sizeof(size_t)),
 * this applies to \p KeyLen.
 *
 * @param ctx (required) pointer to the seos crypto context
 * @param pKeyHandle (required) pointer to key handle
 * @param wrapKeyHandle (optional) handle to existing key for unwrapping
 * @param type (required) type of key imported
 * @param flags (required) flags to set for imported key
 * @param keyData (required) buffer for key material to import
 * @param KeyLen (required) size of key material in buffer
 *
 * @return an error code
 * @retval SEOS_SUCCESS if operation succeeded
 * @retval SEOS_ERROR_INVALID_HANDLE if the object handle is invalid
 ...
 * 
 * The function will also provide the keys' \p flags and \p type parameter.
 *
 * Buffer sizes are limited to approx. 4000 bytes (PAGE_SIZE - sizeof(size_t)),
 * this applies to \p bufSize.
 *
 ...
 * @param type (required) type of exported key
 * @param flags (required) flags of exported key
 * @param buf (required) buffer for key material to export
 * @param bufSize (required) size of buffer, will be set to amount of bytes
